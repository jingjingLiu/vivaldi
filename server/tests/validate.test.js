...
function buildApp() {
    ...
    app.post('/things', validate({ body: z.object({ name: z.string().min(1), age: z.number().int().positive() }) }), (req, res) => res.json({ received: req.body }));
    app.get('/search', validate({ query: z.object({ q: z.string().min(1) }) }), (req, res) => res.json({ q: req.query.q }));
